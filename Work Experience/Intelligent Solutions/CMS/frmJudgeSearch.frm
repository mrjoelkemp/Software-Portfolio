...
Begin VB.Form frmJudgesSearch 
   ...
   Begin MSFlexGridLib.MSFlexGrid sLblista 
      ...
      SelectionMode   =   1
      AllowUserResizing=   1
      FormatString    =   $"frmJudgeSearch.frx":0000
   ...
   End
End
Attribute VB_Name = "frmJudgesSearch"
...
Private Sub cmdNext_Click()
    'Get Judge ID from Table
    g_JudgeID = sLblista.TextMatrix(sLblista.Row, 0)
...
Dim counter As Integer
    ...
    With w_Recordset
        ...
        Do While .EOF = False And counter < 10
            counter = counter + 1
            sLblista.AddItem !judgeid & vbTab & !lastname & vbTab & !firstname & vbTab & !address1 & ", " & !address2
            .MoveNext
        Loop
        If .State = 1 Then
            .Close
...
Function getSQL_Query()
    ...
    If sLastName.Text = "" And sFirstName.Text <> "" Then
        ...
        getSQL_Query = "Select * from judges where firstname >= '" & Trim(sFirstName.Text) & "' ORDER BY firstname ASC"
    
    ElseIf sFirstName.Text = "" And sLastName.Text <> "" Then
        'Query if User defines only Last Name (soft search)
        getSQL_Query = "Select * from judges where lastname >= '" & Trim(sLastName.Text) & "' ORDER BY lastname ASC"
    
    ElseIf sFirstName.Text <> "" And sLastName.Text <> "" Then
        'Query if User defines First Name & Last Name (specific search)
        getSQL_Query = "Select * from judges where firstname = '" & Trim(sFirstName.Text) & "' and lastname = '" & Trim(sLastName.Text) & "' ORDER BY lastname ASC"
    
    ElseIf sJudgeID.Text <> "" And sFirstName.Text = "" And sLastName.Text = "" Then
        'Query if User defines ID only (soft search)
        getSQL_Query = "Select * from judges where judgeid >= '" & Val(sJudgeID.Text) & "' ORDER BY judgeid ASC"
    End If
...
Dim w_Recordset As New Recordset
    ...
    w_SQL = "select * from judges ORDER BY judgeid ASC"
...
Function setFormProperties()
    ...
    isRightScreenToHide ("frmJudgesSearch")
    setCurrentScreen ("frmJudgesSearch")
    frmMDI.hideSelfModuleButton
    
    If g_JudgeMode <> "SEARCH" Then
        g_JudgeMode = g_SystemView
    End If
    
    cmdNext.Caption = "View Details"
    lblWindow.Caption = "Judge - Search"
    
    Select Case g_JudgeMode
        Case "VIEW"
            lblFunction.Caption = "Search For Judge Whose Details You Want To View"
                    
        Case "MODIFY"
            lblFunction.Caption = "Search For Judge Whose Details You Want To Modify"
            
        Case "DELETE"
            lblFunction.Caption = "Search For Judge Whose Details You Wish To Delete"
        
        Case "SEARCH"
            lblFunction.Caption = "Double Click the Desired Judge"
    End Select
...
Private Sub sJudgeID_KeyPress(KeyAscii As Integer)
...
Private Sub sLblista_DblClick()
    If g_JudgeMode = "SEARCH" Then
        ...
        frmCaseNew.sJid = sLblista.TextMatrix(sLblista.Row, 0)
        frmCaseNew.sJfirstname = sLblista.TextMatrix(sLblista.Row, 2)
        frmCaseNew.sJlastname = sLblista.TextMatrix(sLblista.Row, 1)
        Hide
        frmCaseNew.Show
    Else
        'Get Judge ID from Table
