...
Begin VB.Form frmAttorneySearch 
   BorderStyle     =   0  'None
   Caption         =   "Form1"
   ClientHeight    =   8265
   ClientLeft      =   0
   ...
   Begin MSFlexGridLib.MSFlexGrid sLblista 
      ...
      SelectionMode   =   1
      AllowUserResizing=   1
      FormatString    =   $"frmAttorneySearch.frx":0000
   ...
   End
End
Attribute VB_Name = "frmAttorneySearch"
...
'Purpose: Append an ORDER BY clause to SQL query and execute to make
'          entries sort ascending or descending.
Dim w_SQL As String
...
Dim counter As Integer
    ...
    With w_Recordset
        ...
        Do While .EOF = False And counter < 10
            counter = counter + 1
            sLblista.AddItem !attorneyid & vbTab & !lastname & vbTab & !firstname & vbTab & !address1 & ", " & !address2
            .MoveNext
        Loop
        If .State = 1 Then
            .Close
...
Private Sub cmdNext_Click()
    'Get Attorney ID from Table
    g_AttorneyID = sLblista.TextMatrix(sLblista.Row, 0)
...
Dim w_SQL2 As String
    ...
    w_SQL = "Select * from attorneys ORDER BY attorneyid ASC"
...
'Purpose: Return a query based on the fields provided in the search panel
'Rules: 1. Order the entries in ASC order for every search

    If sLastName.Text = "" And sFirstName.Text <> "" Then
        'Query if User defines only First Name (soft search)
        getSQL_Query = "Select * from attorneys where firstname >= '" & Trim(sFirstName.Text) & "' ORDER BY firstname ASC"
    
    ElseIf sFirstName.Text = "" And sLastName.Text <> "" Then
        'Query if User defines only Last Name (soft search)
        getSQL_Query = "Select * from attorneys where lastname >= '" & Trim(sLastName.Text) & "' ORDER BY lastname ASC"
    
    ElseIf sFirstName.Text <> "" And sLastName.Text <> "" Then
        'Query if User defines First Name & Last Name (specific search)
        getSQL_Query = "Select * from attorneys where firstname = '" & Trim(sFirstName.Text) & "' and lastname = '" & Trim(sLastName.Text) & "' ORDER BY lastname ASC"
    
    ElseIf sAttorneyID.Text <> "" And sFirstName.Text = "" And sLastName.Text = "" Then
        'Query if User defines ID only (soft search)
        getSQL_Query = "Select * from attorneys where attorneyid >= '" & Val(sAttorneyID.Text) & "' ORDER BY attorneyid ASC"
    End If
...
Private Sub Form_Activate()
    ...
    Select Case g_AttorneyMode
        Case "VIEW"
            lblFunction.Caption = "Search For Attorney Whose Details You Want To View"
                    
        Case "MODIFY"
            lblFunction.Caption = "Search For Attorney Whose Details You Want To Modify"
            
        Case "DELETE"
            lblFunction.Caption = "Search For Attorney Whose Details You Wish To Delete"
        
        Case "SEARCH"
            lblFunction.Caption = "Double Click the Desired Attorney"
    End Select
...
Function setFormProperties()
    ...
    isRightScreenToHide ("frmAttorneySearch")
    'Set current Screen
    setCurrentScreen ("frmAttorneySearch")
    frmMDI.hideSelfModuleButton
    
    If g_AttorneyMode = g_SystemNew Then
        g_AttorneyMode = g_SystemView
    End If
    
    cmdNext.Caption = "View Details"
    lblWindow.Caption = "Attorney - Search"
...
Private Sub sLblista_DblClick()
    If g_AttorneyMode = "SEARCH" Then
        'Set CaseNew info to the selected entry
        frmCaseNew.sAid.Text = sLblista.TextMatrix(sLblista.Row, 0)
        frmCaseNew.sAfirstname.Text = sLblista.TextMatrix(sLblista.Row, 2)
        frmCaseNew.sALastname.Text = sLblista.TextMatrix(sLblista.Row, 1)
        Hide
        frmCaseNew.Show
